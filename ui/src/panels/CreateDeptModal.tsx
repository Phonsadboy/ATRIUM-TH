...
import type { AccentName, AiProviderId, ModelId, ModelSpeed, ThinkingEffort } from '../contract/types'

const EMOJI_PICKS = ['🟣', '📈', '🤝', '🧪', '🚀', '🛠️', '📣', '📊', '🧠', '💡', '🗂️', '🎯']

export function CreateDeptModal() {
  const open = useUI((s) => s.createDeptOpen)
  const close = useUI((s) => s.closeCreateDept)
  ...
  const [providerId, setProviderId] = useState<AiProviderId>('claude_code')
  const [model, setModel] = useState<ModelId>(DEFAULT_MODEL)
  const [thinkingEffort, setThinkingEffort] = useState<ThinkingEffort>('high')
  const [speed, setSpeed] = useState<ModelSpeed>('standard')
  ...
  const submit = () => {
    if (!canSubmit) return
    const dept = client.createDepartment({
      name: name.trim(),
  ...
  return (
    <Modal open={open} onClose={close} width={620} title="เปิดแผนกใหม่">
      <div className="space-y-3">
        ...
        <Field label="เครื่องมือ (tools) (คั่นด้วยจุลภาค, ไม่บังคับ)">
          <input
            className={inputClass}
            style={{ borderColor: 'var(--color-line-soft)' }}
            value={tools}
            onChange={(e) => setTools(e.target.value)}
            placeholder="เช่น web_search, code_interpreter, ส่งอีเมล"
          />
        </Field>
