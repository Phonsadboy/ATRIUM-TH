...
def _groups() -> list[dict[str, Any]]:
    return [
        ...
        },
        {
            ...
            "credentialId": "openai_embedding_key",
            "providerIds": ["openai"],
            "subsystemId": "embeddings",
            "fields": [
                _field(
                    "ATRIUM_EMBEDDING_PROVIDER",
                    "Embedding provider",
                    kind="select",
                    setting="embedding_provider",
                    placeholder="auto",
                    options=("auto", "local", "openai", "voyage", "hash"),
                    impact="เลือกแหล่ง embedding สำหรับ RAG calls ใหม่ทันที; หลังเปลี่ยน provider/model ควร re-embed stale knowledge",
                ),
                _field(
                    "ATRIUM_OLLAMA_BASE_URL",
                    "Local Ollama URL",
                    setting="ollama_base_url",
                    aliases=("OLLAMA_HOST",),
                    placeholder="http://127.0.0.1:11434",
                    impact="มีผลเมื่อ embedding provider เป็น auto/local",
                ),
                _field(
                    "ATRIUM_OLLAMA_EMBEDDING_MODEL",
                    "Local Ollama model",
                    setting="ollama_embedding_model",
                    placeholder="bge-m3",
                    impact="มีผลกับ local embeddings calls ใหม่ทันที",
                ),
                _field(
                    "ATRIUM_OPENAI_EMBEDDING_API_KEY",
                    "OpenAI embedding key override",
                    kind="secret",
                    setting="openai_embedding_api_key",
                    aliases=("OPENAI_EMBEDDING_API_KEY",),
                    impact="ไม่จำเป็นถ้ามี ATRIUM_OPENAI_API_KEY; มีผลกับ OpenAI embeddings calls ใหม่ทันที",
                ),
                _field(
                    "ATRIUM_OPENAI_EMBEDDING_BASE_URL",
                    "OpenAI embedding base URL",
                    setting="openai_embedding_base_url",
                    aliases=("OPENAI_EMBEDDING_BASE_URL",),
                    placeholder="https://api.openai.com/v1",
                    impact="ถ้าว่างจะใช้ ATRIUM_OPENAI_BASE_URL; มีผลกับ OpenAI embeddings calls ใหม่ทันที",
                ),
                _field(
                    "ATRIUM_OPENAI_EMBEDDING_MODEL",
                    "OpenAI embedding model",
                    setting="openai_embedding_model",
                    placeholder="text-embedding-3-large",
                    impact="ค่าแนะนำคือ text-embedding-3-large; หลังเปลี่ยน model ควร re-embed stale knowledge",
                ),
                _field(
                    "ATRIUM_OPENAI_EMBEDDING_DIMENSIONS",
                    "OpenAI dimensions",
                    kind="number",
                    setting="openai_embedding_dimensions",
                    placeholder="1024",
                    impact="1024 เข้ากับ pgvector/local dim เดิม; ตั้ง 0 เพื่อใช้ native model dim แล้วต้องจัดการ schema ให้รองรับ",
                ),
                _field(
                    "ATRIUM_EMBEDDING_DIM",
                    "Hash/local fallback dim",
                    kind="number",
                    setting="embedding_dim",
                    placeholder="1024",
                    restart_recommended=True,
                    impact="มีผลกับ hash fallback และ pgvector column ตอนเริ่มระบบใหม่",
                ),
            ],
        },
        {
