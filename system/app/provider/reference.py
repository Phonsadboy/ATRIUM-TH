...
def provider_credential_reference(settings: Settings | None = None) -> dict[str, Any]:
    ...
    credentials = [
        ...
        _credential_ref(
            ...
            enables=["Fallback durable image artifact generation through the configured OpenAI-compatible Images API"],
            notes=[
                "This is a subsystem fallback credential, not the primary image route.",
                "ATRIUM_IMAGE_GENERATION_BASE_URL controls the image endpoint; the default is OpenAI-compatible.",
                "ChatGPT OAuth is attempted first; this key is used only when that route cannot produce an image.",
            ],
        ...
        _credential_ref(
            ...
            used_by=["chatgpt_account", "image_generation"],
            enables=[
                "GPT chat/provider calls through the ChatGPT Codex account backend",
                "Primary durable image artifact generation through the ChatGPT OAuth Responses image_generation tool",
            ],
            notes=[
