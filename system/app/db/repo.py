...
def _default_tool_redaction_rules(name: str, risk: str, credentials: bool, external: bool) -> list[str]:
    ...
    if name in {"desktop.screenshot", "browser.screenshot", "browser.open", "browser.click", "desktop.click"}:
...
TOOL_CATALOG = [
    ...
    _tool("video.render_motion", "local_write", mutates=True, timeout=1_800_000, description="Create a Remotion and/or Revideo motion-graphics package from a video timeline, including animated text/caption/overlay layers, template/styleGuide tokens, fonts, safe areas, and commands for preview/render. Use renderer=remotion, revideo, or both. Use asyncMode/background for long package/render work. Automatic render is currently supported for Remotion when dependencies are installed or allowInstall=true.", schema={"projectId": "string", "timeline": "object?", "timelineId": "string?", "version": "number|string?", "renderer": "remotion|revideo|both?", "templateId": "string?", "brandStylePreset": "string?", "brandStyle": "object?", "styleGuide": "object?", "render": "boolean?", "allowInstall": "boolean?", "outputName": "string?", "kind": "preview|final|string?", "timeoutSeconds": "number?", "asyncMode": "boolean?", "background": "boolean?", "waitForResult": "boolean?", "wakeOnComplete": "boolean?"}),
    ...
    _tool("desktop.open_app", "desktop", mutates=True, executor="desktop", description="Open a local desktop application by appName, bundleId, or path. Optionally open a file/url target or pass launch arguments. On Windows the result may include processId for precise activate/quit targeting.", schema={"appName": "string?", "bundleId": "string?", "path": "string?", "target": "string?", "file": "string?", "url": "string?", "newInstance": "boolean?", "arguments": "string[]?", "timeoutSeconds": "number?"}),
    _tool("desktop.activate_app", "desktop", mutates=True, executor="desktop", description="Bring a running desktop application to the foreground before screenshot/click/type/keypress control. On Windows prefer processId when desktop.apps/open_app returned one; activation verifies the target became foreground before later input should be trusted.", schema={"appName": "string?", "bundleId": "string?", "path": "string?", "processId": "number?", "pid": "number?"}),
    _tool("desktop.quit_app", "desktop", mutates=True, executor="desktop", description="Ask a desktop application to quit. force=true escalates to a force-quit/kill operation and should only be used when the app is stuck or the user approved losing unsaved work. On Windows prefer processId to avoid closing unrelated same-name apps.", schema={"appName": "string?", "bundleId": "string?", "path": "string?", "processId": "number?", "pid": "number?", "force": "boolean?", "forceDelaySeconds": "number?", "timeoutSeconds": "number?"}),
