...
TOOL_CATALOG = [
    ...
    _tool("web.search", "network", mutates=False, external=True, executor="web", description="Search the public web without paid API keys. Auto mode uses a configured self-hosted SearXNG instance when present, otherwise DuckDuckGo HTML with Bing RSS as a fallback. Returns ranked titles, URLs, snippets, and source metadata.", schema={"query": "string", "count": "number 1..10?", "provider": "auto|duckduckgo|searxng|bing?", "region": "string?", "safeSearch": "strict|moderate|off?", "categories": "string?", "language": "string?", "engines": "string?", "market": "string?", "mkt": "string?", "timeoutSeconds": "number?"}),
    ...
    _tool("image.generate", "external_send", mutates=True, external=True, executor="openai", timeout=1_800_000, credentials=True, description="Generate image artifacts from text, or from text plus existing image artifact references/masks, using the configured OpenAI-compatible Images API. Queues by default for chat tools so agents can keep working; use asyncMode/statusUrl for background jobs or waitForResult for blocking calls. Prefer gpt-image-2; non-primary GPT image models require user approval or a prior gpt-image-2 failure. Defaults: size=auto, quality=auto, outputFormat=png, background=auto, moderation=auto, n=1. Only send outputCompression when outputFormat is explicitly jpeg or webp; never send it with png or omitted outputFormat.", schema={"prompt": "string", "ownerDept": "string?", "threadId": "string?", "taskIds": "string[]?", "projectId": "string?", "artifactName": "string?", "requestedBy": "string?", "asyncMode": "boolean?", "waitForResult": "boolean?", "wakeOnComplete": "boolean?", "model": "gpt-image-2|gpt-image-2-2026-04-21|gpt-image-1.5|gpt-image-1|gpt-image-1-mini?", "modelOverrideApproved": "boolean?", "modelOverrideReason": "string?", "fallbackFromModel": "gpt-image-2?", "primaryModelError": "string?", "referenceArtifactIds": "string[]?", "maskArtifactId": "string?", "n": "number 1..10?", "size": "auto|WIDTHxHEIGHT?", "width": "number?", "height": "number?", "aspectRatio": "string?", "resolution": "hd|2k|4k?", "quality": "low|medium|high|auto?", "clarity": "draft|standard|final|sharp|string?", "outputFormat": "png|jpeg|webp? default png", "outputCompression": "number 0..100? only with explicit outputFormat jpeg or webp", "background": "auto|opaque|transparent?", "moderation": "auto|low?", "tags": "string[]?"}),
]
